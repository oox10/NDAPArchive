<?php
	...
	*/
	...
	$target_condition = "zong ='議事影音'";
	$meta_exist = array();
	
	ob_start();
	
	$db_update = $db->DBLink->prepare("INSERT INTO resouce_metasimilar VALIES(null,:system_id,:similar_id,:rate,NULL,0,0);");
		  ...
		  while( $meta = $db_select->fetch(PDO::FETCH_ASSOC) ){
			...
			switch($meta['zong']){
			  ...
			  case '議事影音':
				...
				$target_year = substr($source['date_start'],0,4)
			  ...
			  case '檔案': case '議事錄': case '公報': 
				...
				foreach($refer_similar[$meta['identifier']] as $similar){
				  ...
				  $mrefer[$similar['similar_data_id']]['info']  = $rmeta_search['abstract_mask'] ? $rmeta_search['abstract_mask'] : $rmeta_search['abstract'];
