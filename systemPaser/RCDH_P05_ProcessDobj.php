<?php
	...
	*/
	...
	$target_condition = "zong ='活動照片' AND dobj_json='[]'";
	$meta_exist = array();
		  ...
		  while( $meta = $db_select->fetch(PDO::FETCH_ASSOC) ){
			...
			switch($meta['zong']){
			  ...
			  case '活動照片':
				...
				//"photo_from": "0019", "photo_end": "0021",
				if($source['photo_from'] && $source['photo_end']){
				
   				  $range_flag = 0;
				  foreach($file_list as $i=>$file_name){
					
					if(!$range_flag){
					  if(preg_match('/'.$source['photo_from'].'\./',$file_name)){
						$range_flag = 1;
					  }	
					}
					...
					if($range_flag){
					  ...
					  if(preg_match('/'.$source['photo_end'].'\./',$file_name)){
					  	$range_flag = 0;
					    break;
					  }
					} 
