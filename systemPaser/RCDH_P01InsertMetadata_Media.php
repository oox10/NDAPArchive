<?php
	...
	*/
	...
	try{ 
	  ...
	  $db_select = $db->DBLink->prepare("SELECT * FROM ".$source_table." WHERE ".$target_condition." ORDER BY  record_pkid ASC;");
	  ...
	  while( $source = $db_select->fetch(PDO::FETCH_ASSOC) ){
		...
		$collection_id = substr($source['StoreNo'],0,17);
